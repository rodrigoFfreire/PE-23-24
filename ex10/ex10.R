...
qF <- function(p, a, b) {
  ...
  inverse_cdf <- function(prob) {
    uniroot(function(x) F(x, a, b) - prob, interval = c(a,b))$root
  }
  
  q <- sapply(p, inverse_cdf)
  return(q)
}

probs <- 0:k / k

limites <- qF(probs, a, b)

classes <- cut(data, breaks = limites)

freq_observadas <- table(classes)

solution <- chisq.test(freq_observadas, p = rep(1 / k, k))$p.value
